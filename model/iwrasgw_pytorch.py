...
def iwrasgw_gpu(xs,xt,device, nproj=200,tolog=False,P=None):
...
def weighted_distance(directions_distances):
    """
    Compute the weighted sum of distances using softmax weights for each projection direction.
    
    Parameters:
    ----------
    directions_distances : tensor, shape (n_proj, 1)
        Distances for each projection direction.

    Returns:
    -------
    weighted_distance : tensor, shape (1,)
        The weighted sum of distances.
    """
    # Flatten the directions_distances if it has a second dimension (n_proj, 1)
    directions_distances = directions_distances.flatten()
    
    # Apply softmax to the flattened directions_distances tensor
    weights = torch.softmax(directions_distances, dim=0)  # Softmax over distances to get weights
    
    # Compute the weighted sum of distances
    weighted_distance = torch.sum(directions_distances * weights)  # Weighted sum of distances
