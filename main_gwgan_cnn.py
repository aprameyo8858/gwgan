...
#from model.rasgw_pytorch_loop_L import rasgw_gpu
from model.fid import calculate_fid_inception
...
save_fig_path = 'trial_out_iwrasgw' + model
...
#inception_model = models.inception_v3(pretrained=True).float()
...
def tensor_to_pil_image(tensor):
    # Ensure tensor is float32 and values are in the range [0, 1]
    tensor = tensor.float()
...
#resnet_model = models.resnet18(pretrained=True).to(torch.float32)
#resnet_model.eval()
...
def calculate_fid(real_images, generated_images, device='cuda'):
    """
    Calculate FID score between real and generated images using MobileNetV2.
    ...
    # Function to get features from MobileNetV2 after transforming the images
    def get_features(images):
        ...
        images_resized = torch.stack(images_resized).to(device).float()

        # Forward pass through the MobileNetV2 model
        with torch.no_grad():
            features = resnet_model(images_resized)
            # Apply global average pooling to get feature vectors
            #pooled_features = adaptive_avg_pool2d(features, (1, 1)).squeeze()
    ...
    # Regularize covariance matrices by adding a small epsilon value to the diagonal
    epsilon = 1e-6
    ...
    if torch.any(torch.isnan(sigma_real)) or torch.any(torch.isnan(sigma_generated)):
        print("NaN detected in covariance matrices!")
    # Calculate the FID score
    fid_score = torch.norm(mu_real - mu_generated) ** 2 + \
                torch.trace(sigma_real + sigma_generated -
                            2 * torch.sqrt(torch.mm(sigma_real, sigma_generated)))
...
def calculate_fid_inception_(real_images, generated_images, device='cuda'):
    ...
    # Ensure model is on the right device
    inception_model.to(device).float()
...
for epoch in range(num_epochs):
    ...
    for it, (image, _) in enumerate(dataloader):
        ...
        if epoch == num_epochs-1:        #num_epochs - 1
                ...
                if it % 20 == 0:
                    fid_score = calculate_fid_inception(real_images, generated_images, device='cuda')
                    ...
                    print("fid score:",fid_score,"recon loss:",reconstruction_loss.item())
        ...
        #trial
        #loss = sgw_gpu_original(D_x.to('cuda'), D_g.to('cuda') ,'cuda',nproj=500,tolog=False,P=None)
        #loss = rasgw_gpu(D_x.to('cuda'), D_g.to('cuda') , 'cuda',L=10, nproj=500,tolog=False,P=None)
        #trial
        ...
        #loss = rasgw_gpu(f_x.to('cuda'), f_g.to('cuda') , 'cuda',L=100, nproj=50000,tolog=False,P=None)
        loss = iwrasgw_gpu(D_x.to('cuda'), D_g.to('cuda') , 'cuda',L=10, nproj=500,tolog=False,P=None)
    ...
    #colorbar(ax0)
    #ax1 = ax[1].imshow(f_x.cpu().detach().numpy(), cmap='Blues')
    #colorbar(ax1)
    #ax2 = ax[2].imshow(_g.cpu().detach().numpy(), cmap='Blues')
    #colorbar(ax2)
    #ax[0].set_title(r'$T$')
...
if reconstruction_losses_last_epoch:
    ...
    variance_fid = np.std(fid_scores_last_epoch)

    print(f"\nMean FID Score (Epoch {num_epochs}): {mean_fid}")
    print(f"Standard deviation of FID Score (Epoch {num_epochs}): {variance_fid}")
        
    mean_loss = np.mean(reconstruction_losses_last_epoch)
    variance_loss = np.std(reconstruction_losses_last_epoch)

    print(f"Mean Reconstruction Loss (Epoch {num_epochs}): {mean_loss}")
    print(f"Standard deviation of Reconstruction Loss (Epoch {num_epochs}): {variance_loss}")
