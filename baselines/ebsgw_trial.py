...
def gromov_1d(xs, xt, L = 10, tolog=False):
    """Solves the Gromov in 1D (eq (2) in [1] for each proj). Returns L distances."""
    ...
    # Compute L distances by taking minimum of l1 and l2 for each projection
    distances = torch.min(l1, l2)

    # Ensure that we return L distances. If distances < L, replicate the values.
    if distances.size(0) < L:
        distances = distances.repeat(L)

    # Now, we slice the distances to return only the first L values
    ...
    if tolog:
        return distances, log
    else:
        return distances


# Gromov-1D function that computes the 1D Gromov Wasserstein distance between sorted projections
def gromov_1d_(xs, xt, tolog=False):
...
# Empirical Barycentric Sliced Gromov-Wasserstein (EB-SGW) function
def EBSGW(X, Y, L=10, p=2, device='cuda', nproj=200):
    """Computes the Empirical Barycentric Sliced Gromov-Wasserstein (EB-SGW) distance."""
    dim = X.size(1)  # Dimension of the data

    # Initialize the projections (theta)
    theta = rand_projections(dim, L, device)

    # Apply projections using sink_ for both source and target
    xsp, xtp = sink_(X, Y, device, nproj=nproj)

    # Compute the sliced Gromov-Wasserstein distances between the source and target
    gromov_distances = gromov_1d(xsp, xtp, L ,  tolog=False )

    # Reshape and apply softmax to get the barycentric weights
    gromov_distances = gromov_distances.view(1, L)
    weights = torch.softmax(gromov_distances, dim=1)

    # Compute the final EB-SGW distance
    sw = torch.sum(weights * gromov_distances, dim=1).mean()
