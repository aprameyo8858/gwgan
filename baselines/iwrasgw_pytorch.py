...
def iwrasgw_gpu(xs,xt,device,L=10, nproj=200,tolog=False,P=None):
    ...
    for i in range (L):
        ...
        rasgw_costs.append(d.item())
    rasgw_costs = torch.tensor(rasgw_costs, device = device)
    weights = torch.softmax(rasgw_costs, dim = 0)
    d = torch.sum(weights*rasgw_costs)
    if tolog:
        return d,log
    else:
        return d
